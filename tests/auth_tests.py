...
from tests.config_tests import TestingSessionLocal
...
def test_signup(client, monkeypatch):
    ...
    monkeypatch.setattr("src.routers.auth.send_confirm_email", mock_send_email)
...
async def test_confirm_email(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.routers.auth.get_email_from_token", mock_get_email_from_token
    )
    ...
    monkeypatch.setattr("src.routers.auth.UserService", lambda db: mock_user_service)
...
async def test_confirm_reset_password(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.routers.auth.get_password_from_token", mock_get_password_from_token
    )
